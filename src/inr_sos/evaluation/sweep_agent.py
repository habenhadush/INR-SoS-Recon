"""
sweep_agent.py  (unified, data-driven)
---------------------------------------
...
The sweep search space is read from datasets.yaml so new domains
(k-wave, phantom, clinical) only require a yaml entry — no code changes.
...
# Path to datasets.yaml (relative to this file → ../../scripts/)
_DATASETS_YAML = Path(__file__).resolve().parents[3] / "scripts" / "datasets.yaml"


# ──────────────────────────────────────────────────────────────────────────────
# YAML → W&B sweep config builder
# ──────────────────────────────────────────────────────────────────────────────

def _load_dataset_sweep_config(dataset_key: str = None) -> dict:
    """Load the sweep.parameters section from datasets.yaml."""
    with open(_DATASETS_YAML) as f:
        cfg = yaml.safe_load(f)
    key = dataset_key or cfg["active"]
    ds = cfg["datasets"][key]
    sweep_section = ds.get("sweep", {})
    if not sweep_section:
        raise ValueError(
            f"Dataset '{key}' has no 'sweep' section in datasets.yaml. "
            f"Add a sweep config to run sweeps on this dataset."
        )
    return sweep_section


def _coerce_numeric(v):
    """Convert string-encoded numbers (e.g. '1e-5') to float.

    YAML safe_load parses scientific notation like 1e-5 as strings.
    W&B needs actual floats for min/max/values.
    """
    if isinstance(v, str):
        try:
            return float(v)
...
def _yaml_param_to_wandb(value) -> dict:
    """Convert a yaml parameter entry to W&B sweep format.
...
def get_sweep_config(
    ...
    """
    Build a W&B Bayesian sweep config from datasets.yaml.

    Parameters
    ----------
    dataset_key : str or None
        Key in datasets.yaml (e.g. "inverse_crime", "kwave_geom").
        If None, uses the 'active' field from the yaml.
    metric_goal : str
        W&B metric name to optimise.
    metric_direction : str
        "minimize" or "maximize".

    Returns
    -------
    dict : ready to pass to wandb.sweep()
    """
    sweep_cfg = _load_dataset_sweep_config(dataset_key)
    params_yaml = sweep_cfg.get("parameters", {})

    # Convert each yaml param entry to W&B format
    wandb_params = {}
    for param_name, param_value in params_yaml.items():
        wandb_params[param_name] = _yaml_param_to_wandb(param_value)

    key_label = dataset_key or "active"
    return {
        "method": "bayes",
        "name": f"sweep_{key_label}",
        "metric": {
            "name": metric_goal,
...
def run_sweep_agent(
    ...
    def _sweep_train_fn():
        ...
        # K-wave / robust training params (new — backward compatible)
        cfg.loss_type      = sc.get("loss_type",      cfg.loss_type)
        cfg.huber_delta    = sc.get("huber_delta",     cfg.huber_delta)
        cfg.clamp_slowness = sc.get("clamp_slowness",  cfg.clamp_slowness)
        cfg.early_stopping = sc.get("early_stopping",  cfg.early_stopping)
        cfg.patience       = sc.get("patience",        cfg.patience)
        cfg.val_fraction   = sc.get("val_fraction",    cfg.val_fraction)
        cfg.time_scale     = sc.get("time_scale",      cfg.time_scale)
