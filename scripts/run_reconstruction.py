...
ENGINE_MAP = {
    ...
    "SVD_Constraint":  optimize_with_svd_constraint,
}
MODEL_MAP = {
...
def run_inr_config(sweep_cfg, dataset, indices, base_config, run_tag, log,
    ...
    for i, idx in enumerate(indices):
        ...
        if method == "Bias_Absorption":
            # For Bias Absorption, we need a secondary model
            bias_model = BiasMLP(
                in_features=2, 
...
def _plot_absorbed_bias(epsilon_bias, idx, method, mtype, plot_dir):
    ...
    eps_np = (epsilon_bias.detach().cpu().numpy() 
              if hasattr(epsilon_bias, "detach") 
              else np.asarray(epsilon_bias))
