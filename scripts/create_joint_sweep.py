#!/usr/bin/env python3
"""
Create a W&B Bayesian sweep for the joint denoiser+reconstructor pipeline.

Usage:
    python create_joint_sweep.py --dataset kwave_geom --n_runs 150
    python create_joint_sweep.py --dataset kwave_blob --n_runs 150
"""

import argparse
import json
import yaml
import wandb
from datetime import datetime
from pathlib import Path

from inr_sos.evaluation.joint_sweep_agent import get_joint_sweep_config

_LOG_FILE = Path(__file__).parent / "sweep_registry.json"
_DATASETS = Path(__file__).parent / "datasets.yaml"


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--dataset", default=None,
                        help="Dataset key from datasets.yaml (default: uses 'active' field)")
    parser.add_argument("--project", default="INR-SoS-Recon")
    parser.add_argument("--n_runs", default=150, type=int)
    parser.add_argument("--metric", default="MAE_mean")
    parser.add_argument("--direction", default="minimize")
    args = parser.parse_args()

    with open(_DATASETS) as f:
        ds_yaml = yaml.safe_load(f)
    dataset_key = args.dataset or ds_yaml["active"]
    dataset_name = ds_yaml["datasets"][dataset_key]["name"]

    cfg = get_joint_sweep_config(
        dataset_key=dataset_key,
        metric_goal=args.metric,
        metric_direction=args.direction,
    )

    sweep_id = wandb.sweep(cfg, project=args.project)
    entity = wandb.api.default_entity

    url = f"https://wandb.ai/{entity}/{args.project}/sweeps/{sweep_id}"

    entry = {
        "sweep_id": sweep_id,
        "project": args.project,
        "entity": entity,
        "dataset": dataset_key,
        "sweep_type": "joint",
        "n_runs": args.n_runs,
        "metric": args.metric,
        "direction": args.direction,
        "created_at": datetime.now().isoformat(),
        "status": "created",
        "url": url,
    }

    registry = []
    if _LOG_FILE.exists():
        with open(_LOG_FILE) as f:
            registry = json.load(f)

    registry.append(entry)
    with open(_LOG_FILE, "w") as f:
        json.dump(registry, f, indent=2)

    print("\n" + "=" * 60)
    print(f"  Joint sweep created")
    print(f"  ID      : {sweep_id}")
    print(f"  Dataset : {dataset_name} ({dataset_key})")
    print(f"  Project : {args.project}")
    print(f"  N runs  : {args.n_runs}")
    print(f"  URL     : {url}")
    print("=" * 60)
    print(f"\nNext step — launch the agent in tmux:")
    print(f"\n  tmux new -s jsweep_{sweep_id[:6]}")
    print(f"  python run_joint_sweep.py --sweep_id {sweep_id} --n_runs {args.n_runs} --dataset {dataset_key}")
    print(f"  # Then Ctrl+B D to detach\n")


if __name__ == "__main__":
    main()
